...
httpsServerOptions.secureOptions = constants.SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION;
...
if (req.socket && cookies) {
    ...
    if (bannerCookie == -1) {
		...
		//  Show user Logon Page or XUI
			...
			req.connection.renegotiate(clientAuthOpts, function(err) {
				if (err) {
					console.log(err);
				} else {
